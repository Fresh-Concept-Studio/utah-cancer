...
- `legacy-url-inventory.csv` records all 213 URLs from the old WordPress sitemaps. Eleven older patient-resource URLs remain for a content decision, documented in `patient-resource-content-review.md`.
...
- Do not redirect the 11 remaining review URLs to the homepage as a blanket rule. Use the recommendations in `patient-resource-content-review.md` to preserve useful content, map it to a genuinely equivalent page, or return `410` after review.
